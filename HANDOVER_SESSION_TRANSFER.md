...
**РЕШЕНИЕ ВЛАДЕЛЬЦА (25.08): линии объединяются в одну.** Отдельного портала
воркера больше нет; его функционал перестраивается внутри админки сайта отдельной
вкладкой «Воркер». Основа объединения — **линия B**: проверено пофайлово, что её
дерево уже содержит работу линии A по существу (MANAGER_EMAILS, чек-лист и
таймлайн T1–T4, загрузка файлов, Google OAuth, guard чанков, UK/EN, хаб
Експансія, `/pricing`, безбраузерный пререндер, приём лидов в Supabase), а сверх
того несёт админку на 3012 строк с ролями, конструктор аудита, коннекторы
GA4/GSC/PageSpeed и дата-рум. Единственный файл, уникальный для линии A, —
`HANDOVER_SESSION_TRANSFER.md`. Потерь при объединении нет.

⚠️ Статус: сам мерж **не выполнен** — команда `git merge` отклонена
классификатором разрешений среды. Нужна отмашка владельца или правило в
настройках. Команда: `git merge line-b -X theirs`, проверка результата —
`git diff --stat line-b HEAD` должен показать только HANDOVER-файл.
...
## 14. Векторы развития (со слов владельца, 25.08)

Четыре направления. Порядок — как продиктован, приоритет владельцем не расставлен.

**1. Фронтенд weexp.agency и сайт в целом.** Публичная часть: доработки,
контент, вёрстка. Правила §7 действуют без исключений.

**2. «Глубокий аудит» как услуга — автоматизировать по максимуму.** Это то, что
weexp продаёт клиентам, и развивается оно **на базе сайта** (кабинет клиента +
админка), а не в отдельном инструменте. Задел в линии B: вкладка «Глибокий аудит»,
конструктор аудита (`AuditBuilder`, `AuditForm`, `AuditPackPage`, `auditTemplate`),
проекты (`ProjectView`), чек-лист файлов, выгрузка опросника в PDF, коннекторы
GA4/GSC/PageSpeed для сбора фактуры.

**3. Воркер — личный инструмент администратора.** ❗ Не движок клиентской услуги.
Узконаправленные персональные задачи владельца: прогнать отдельную линзу по
произвольному сайту, быстро собрать факты, проверить гипотезу. Живёт отдельной
вкладкой «Воркер» в админке.
Готовые линзы в `worker/src/`: `seoarch`, `techaudit`, `contentaudit`, `uxui`,
`competitor`, `intelligence`, `channels`, `mechanics`, `journey`, `visionAudit`,
`designReview`, `externalAudits`, `maturity`, `pagereport`.
Мост уже есть: `api/audit-run.js` — серверный прокси admin → воркер, держит токен
на сервере, действия `health` · `start` · `status` · `learn` · `pack`.

**4. Админка weexp.** Сама панель как продукт: роли и права (`cap:`), вкладки
Дашборд · Заявки · Користувачі · Аудити · Глибокий аудит · Проекти · Налаштування,
плюс будущая вкладка «Воркер».

**Техническая рамка, принятая при разборе.** «Нет отдельного портала воркера» =
убирается **консоль оператора** (`worker/src/console.ts`), а сам движок остаётся
сервисом на Railway за `/api/audit-run`: Playwright-обход и прогон на 15+ отчётов
не помещаются в лимиты serverless-функций Vercel. Если владелец имел в виду отказ
от Railway целиком — это отдельное решение с отдельной ценой, не подразумевай его.
