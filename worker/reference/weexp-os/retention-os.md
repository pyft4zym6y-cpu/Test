...
## 02 · Objects
Владеет: `Lifecycle` (сегмент и его триггерная программа: правила входа, коммуникации, выхода)
`Customer` остаётся у data-os — мы не владеем клиентом, мы владеем программой работы с сегментом.
...
## 13 · Аудит-модуль
**Модуль 12 · Повторные покупки.** Основной слой доступа: **L2**.
Частичный L0: есть ли подписка и что приходит после неё, есть ли приветственная цепочка, программа лояльности, брошенная корзина (проверяется тестовым заказом), присутствие в мессенджерах, работа с отзывами после покупки.
L0 частично: подписка на рассылку, наличие программы лояльности и приветственной цепочки проверяются снаружи.
Чек-лист L2: матрица триггеров против эталона; сегментация и RFM; когорты и LTV; доля повторных покупок; реактивация; частота и пересечение коммуникаций.
Выход в отчёт: карта триггеров с пропусками, когортные кривые, оценка неиспользованного резерва — у производителя с повторяемым товаром это часто крупнейший резерв в проекте.
## 14 · AI Layer
