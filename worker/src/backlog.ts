...
export function renderBacklogHtml(r: BacklogReport): string {
  ...
  const cover = `<section class="cover"><div class="cov-bar"></div><div class="cov-body">
    <div class="kicker">Commerce OS · Сводный бэклог работ · слой A0</div>
    <h1>${esc(r.verdict)}</h1>
    <div class="cov-meta">
  ...
  const meth = methodologySection({
    ...
    limits: 'Оценки эффекта — отраслевые ориентиры; бюджеты и сроки уточняются при составлении сметы. Работы вне витрины (кабинеты, CRM) появляются после доступов A1–A2.',
  });
  ...
  const foot = `<section class="block"><div class="footer">Commerce OS · Сводный бэклог A0 · ${esc(r.client)} · ${esc(date)}. Единственный источник приоритетов пакета; частные списки в документах — доказательная база, не конкурирующие планы.</div></section>`;
  ...
  return doc(`Сводный бэклог A0 · ${r.client}`, cover + meth + table + concl + foot, extra);
}
