...
export async function runJourney(browser: Browser, origin: string, log?: (m: string) => void): Promise<JourneyStep[]> {
  ...
  let n = 0;
  const add = (stage: string, action: string, expected: string, result: string, status: StepStatus, dims: Dim[]) =>
    steps.push({ n: ++n, stage, action, expected, result, status, dims });
  ...
  try {
    ...
    } catch (e) { add('Поиск', 'Пробую поиск', 'Выдача с товарами', `сбой: ${String(e).slice(0, 80)}`, 'спотыкание', ['UX']); }
    ...
    } catch (e) { add('Каталог', 'Перехожу в каталог', 'Листинг открывается', `сбой: ${String(e).slice(0, 80)}`, 'спотыкание', ['UX']); }

    /* 4 · Карточка из листинга */
    let onPdp = false;
    try {
      ...
      if (await prod.count()) {
        ...
        await page.waitForLoadState('domcontentloaded').catch(() => {});
        await page.waitForTimeout(1000);
        onPdp = (await page.locator(ADD_TO_CART).count()) > 0 || await page.evaluate(() => /в корзину|в кошик|додати|купити|купить/i.test(document.body?.textContent ?? '')).catch(() => false);
      }
      add('Карточка', 'Открываю первый товар из листинга', 'Карточка с ценой и кнопкой «в корзину»', onPdp ? 'карточка открылась, кнопка покупки на месте' : 'карточка не открылась или без кнопки покупки', onPdp ? 'пройден' : 'тупик', ['UX', 'CRO']);
    } catch (e) { add('Карточка', 'Открываю товар', 'Карточка с кнопкой покупки', `сбой: ${String(e).slice(0, 80)}`, 'спотыкание', ['UX']); }
    ...
    if (onPdp) {
      ...
      } catch (e) { add('Добавление в корзину', 'Нажимаю «в корзину»', 'Явное подтверждение добавления', `сбой: ${String(e).slice(0, 80)}`, 'тупик', ['CRO']); }
      ...
      } catch (e) { add('Избранное', 'Пробую избранное', 'Товар сохраняется', `сбой: ${String(e).slice(0, 80)}`, 'спотыкание', ['UX']); }
    } else {
      add('Добавление в корзину', '—', 'Явное подтверждение добавления', 'шаг недостижим: карточка не открылась', 'не проверялся', ['CRO']);
      add('Избранное', '—', 'Сохранение на потом', 'шаг недостижим: карточка не открылась', 'не проверялся', ['UX']);
    }
    ...
    } catch (e) { add('Корзина', 'Открываю корзину', 'Состав и сумма читаются', `сбой: ${String(e).slice(0, 80)}`, 'тупик', ['CRO']); }
    ...
    if (cartOk) {
      ...
      } catch (e) { add('Чекаут', 'Перехожу к оформлению', 'Форма оформления открывается', `сбой: ${String(e).slice(0, 80)}`, 'тупик', ['CRO']); }
    } else {
...
export function buildJourneyReport(steps: JourneyStep[], clientUrl: string, takenAt: string): JourneyReport {
  ...
  const recommendations = [
    ...steps.filter((s) => s.status === 'тупик').map((s) => ({ pr: 'P0' as const, action: `${s.stage}: устранить тупик — ${s.expected.toLowerCase()}`, effect: 'Разблокирует шаг воронки, на котором покупатель уходит' })),
    ...steps.filter((s) => s.status === 'спотыкание').map((s) => ({ pr: 'P1' as const, action: `${s.stage}: убрать трение — ${s.expected.toLowerCase()}`, effect: 'Снижает потери на шаге' })),
  ...
  const verdict = !steps.length ? 'Путь клиента не пройден — сайт недоступен.'
    : deadends === 0 && friction <= 1 ? `Путь до чекаута проходим: ${passed}/${steps.length} шагов чисто, критических обрывов нет.`
    : deadends === 0 ? `Путь проходим, но с трением: ${friction} шагов заставляют покупателя думать или сомневаться.`
    : `Путь клиента рвётся: ${deadends} тупиков до оформления заказа — часть покупателей физически не доходит до оплаты.`;

  const worst = steps.find((s) => s.status === 'тупик') ?? steps.find((s) => s.status === 'спотыкание');
  const conclusion = [
