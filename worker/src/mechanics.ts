...
export function buildMechanics(ds: AuditDataset): MechanicsReport {
  ...
  add(G4, 'Внешняя репутация (отзывы вне сайта)', 'Ссылки на профили отзывов: карты, Trustpilot, агрегаторы', extRep ? true : null, extRep ? 'ссылки на внешние профили отзывов найдены' : 'ссылок на внешние профили не видно — проверить репутацию вручную (A1)', 'Доверие новичка формируется до захода на сайт', 'P1', ['COMM', 'MKT']);
  ...
  /* ── 6 · Ценообразование (проектируется в ассортименте — внешне почти не видно) ── */
  const G6 = 'Ценообразование';
  add(G6, 'Ценовое якорение', 'Старая/зачёркнутая цена и «выгода» рядом с ценой', ptFound('sale') || block('urgency') ? true : null, ptFound('sale') ? 'скидочный контур найден' : 'полнота якорения — вручную', 'Восприятие выгоды без реального демпинга', 'P1', ['PRICE', 'CRO']);
  add(G6, 'Good / Better / Best', 'Три уровня товара/комплекта для сдвига выбора к среднему', null, 'проектируется в ассортименте — внешне не измеряется', 'Управляемый сдвиг чека к целевому варианту', 'P2', ['PRICE', 'COMM']);
  add(G6, 'Декой (приманка)', 'Намеренно невыгодный вариант, оттеняющий целевой', null, 'проектируется — внешне не видно', 'Рост доли целевого варианта в выборе', 'P2', ['PRICE']);
  ...
  const ECON_GATED = new Set(['Бонусные баллы / кэшбек', 'Количественные скидки', 'Наборы / бандлы', 'Программа лояльности (уровни)', 'Оплата частями / рассрочка', 'Промокоды', 'Ценовое якорение']);
  for (const r of rows) if (ECON_GATED.has(r.name)) r.effect += ' · приоритет условный: до внедрения проверить маржу, repeat rate, частоту и чувствительность к скидке (benchmark ≠ гарантированный эффект)';
  ...
  const conclusion = [
    ...
    'Важно про приоритеты: столбец «Эффект» — отраслевые ориентиры, а не обещанный результат (benchmark ≠ ожидаемый эффект для этого магазина). Механики, зависящие от юнит-экономики (кэшбек, скидки, бандлы, рассрочка, лояльность, якорение), помечены — их приоритет условный до проверки маржи, repeat rate, частоты покупок и чувствительности к скидке. Иначе легко поднять retention, но убить contribution margin: например, кэшбек при низкой марже и редких повторных покупках уводит юнит-экономику в минус.',
  ];
