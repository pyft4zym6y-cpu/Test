...
export function renderChannelsHtml(r: ChannelsReport): string {
  ...
  const cover = `<section class="cover"><div class="cov-bar"></div><div class="cov-body">
    <div class="kicker">Commerce OS · Аудит каналов · внешний аудит витрины</div>
    <h1>${esc(r.verdict)}</h1>
    <div class="cov-meta">
    ...
    <div class="coverage"><b>Что видно во внешнем аудите:</b> только то, что «зашито» в сайт — трекинг, соцсети, retention-блоки. Фактическая эффективность каналов (платка, email, маркетплейсы) требует доступа к кабинетам и данным — после подключения аналитики и рекламных кабинетов (${r.blocked} канала со статусом «Нужны доступы»). Отсутствие данных не выдаётся за факт.</div>
  </div></section>`;
  ...
  const meth = methodologySection({
    ...
    limits: 'Внешний аудит видит только то, что встроено в сайт. Расходы, ROAS и доля выручки по каналам живут в кабинетах — после подключения аналитики и рекламных кабинетов. Канал без внешних сигналов ≠ канал не работает, но это повод проверить.',
  });
  ...
  const concl = conclusionSection([
    ...
    `Оценка эффективности (расходы, ROAS, доля выручки) на этом слое невозможна и не имитируется: ${blockedRows.map((x) => x.channel.toLowerCase()).join(', ')} — закрываются доступами после подключения аналитики и рекламных кабинетов.`,
  ], 'Следующий этап: доступ к GA4 и рекламным кабинетам — фактическая экономика каналов, после чего этот отчёт превращается из карты инфраструктуры в карту эффективности.');

  const foot = `<section class="block"><div class="footer">Commerce OS · Аудит каналов · ${esc(r.client)} · ${esc(date)}. Внешний аудит витрины: внешние сигналы. Отсутствие данных не выдаётся за факт и не скрывается; эффективность каналов закрывается после подключения аналитики и рекламных кабинетов.</div></section>`;
  ...
  return doc(`Аудит каналов · ${r.client}`, cover + meth + table + swSection(strengths, weaknesses) + recsSection(recs) + concl + foot, extra);
}
