...
export function buildContentAudit(ds: AuditDataset): ContentReport {
  ...
  const conclusion = !rows.length ? ['Аудит контента не выполнен: ни одна страница не была разобрана. Это блокер, а не вывод о контенте.'] : [
    ...
    `Вывод сформирован по наличию решающих блоков (внешний аудит витрины, наблюдение). Он отвечает на вопрос «есть ли на странице то, что нужно для решения», но не «насколько хорошо это написано» — качество текста, уникальность и тональность проверяются после передачи доступов (следующий этап) с доступом к контенту и Search Console.`,
  ];
