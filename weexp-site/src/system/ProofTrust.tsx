...
export function ProofTrust() {
  ...
  const FAQ = [
    ...
    { q: t('Як влаштований договір і відповідальність?', 'How are the contract and liability handled?'),
      a: t('Працюємо офіційно: WEEXP — європейська компанія, зареєстрована в ЄС, із документообігом і рахунками в євро. До старту — договір із предметом, строками, Definition of Done і порядком приймання. Обсяг і KPI узгоджуємо письмово, доступи ви відкриваєте контрольовано й у будь-який момент можете відкликати.', 'We operate officially: WEEXP is a European company registered in the EU, with EU paperwork and invoicing in euro. Before kickoff — a contract with scope, timelines, a Definition of Done and an acceptance procedure. Scope and KPIs are agreed in writing, you grant access in a controlled way and can revoke it at any time.') },
