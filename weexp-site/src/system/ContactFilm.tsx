        ...
        {status === 'ok' ? (
          ...
          <div className="sysx-card">
            ...
            <form className="ctf-form" onSubmit={submit}>
              ...
              {status === 'error' && (
                <p className="sysx-note mono ctf-full" style={{ color: '#F5301C' }}>{t('Не вдалося надіслати заявку. Спробуйте ще раз або напишіть нам на ', 'Could not send the request. Please try again or write to us at ')}<a href={`mailto:${MAIL}`}>{MAIL}</a>.</p>
              )}
            </form>
          </div>
        )}
