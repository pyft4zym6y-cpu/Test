...
export const PHASES: { n: Phase; l: string; note: string }[] = [
  // Кваліфікація живе в заявці (CRM), а не тут: дві воронки з тим самим
  // питанням «беремось чи ні» і породжували питання «де джерело правди».
  // Фаза 0 — це запит на глибокий аудит від клієнта, з яким уже працюють.
  { n: 0, l: 'Запит на аудит', note: 'клієнт попросив глибокий аудит — відкриваємо доступ' },
  { n: 1, l: 'Етап 1 · Аудит', note: 'збираємо базу знань і ставимо діагноз' },
  { n: 2, l: 'Етап 2 · Впровадження', note: 'проєкт створено із заявки, працюємо за роадмапою аудиту' },
