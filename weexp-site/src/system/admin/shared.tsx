...
/**
 * Тип роботи — ДРУГИЙ вимір, не статус.
 *
 * Стадія відповідає на «де заявка в процесі», тип — на «про що вона». Це різні
 * питання, і змішувати їх в одному списку означає щоразу вибирати між ними.
 * «Аудит» розділено надвоє: безкоштовний експрес і платний глибокий — по суті
 * різні продукти, які раніше рахувались одним рядком.
 */
export const COOP_TYPES: { k: string; l: string }[] = [
  { k: 'express', l: 'Безкоштовний аудит' },
  { k: 'deep', l: 'Глибокий аудит' },
  { k: 'consulting', l: 'Консалтинг' },
  { k: 'full', l: 'Повний супровід' },
];
/** Старі ключі в записах: 'audit' був платним глибоким, 'other' — без типу. */
const COOP_LEGACY: Record<string, string> = { audit: 'deep', other: '' };
export const coopOf = (k?: string): string => (k ? (COOP_LEGACY[k] ?? k) : '');
export const coopLabel = (k?: string) => COOP_TYPES.find((c) => c.k === coopOf(k))?.l || '';
/**
 * Стадія заявки. Легасі-статуси зводяться до пʼяти чинних: 'done'
 * («Завершена») стадією більше не є — заявку або довели до проєкту (це видно з
 * projectId), або закрили в архів. Окремої «Завершеної» між цими двома
 * станами немає, і саме її двозначність і плутала: «завершена» читалась і як
 * «виграли», і як «закрили».
 */
export const stageOf = (l: LeadRow): LeadStatus => {
  const s = l.status || 'new';
  if (s === 'proposal') return 'progress';
  if (s === 'won' || s === 'done') return 'archive';
...
/**
 * Послуга одна — «Глибокий аудит» (ключ DEEP). Рівнів T1–T4 у клієнтському
 * інтерфейсі немає: усередині одного аудиту є Питання · Доступи · Файли, а
 * поділ на рівні лишився внутрішньою методологією команди.
 *
 * У старих записах ключі T1–T4 ще трапляються. Показувати їх «як є» означало
 * повертати скасовану модель на екран одним рядком даних, тож вони підписані
 * архівом, а не кодом.
 */
export const isLegacyTier = (tid: string) => /^T\d+$/i.test(tid);
export const tierLabel = (tid: string) =>
  tid === 'DEEP' ? 'Глибокий аудит' : isLegacyTier(tid) ? 'Архівний запит (стара модель рівнів)' : tid;
...
export type UTab = 'over' | 'comp' | 'express' | 'deep' | 'proj' | 'files' | 'kb' | 'inner' | 'team';
/**
 * Вкладки картки клієнта відповідають на пʼять питань менеджера — у тому
 * порядку, в якому вони виникають:
 *
 *   хто клієнт            → Огляд, Компанія
 *   що він уже зробив     → Експрес-аудит
 *   що замовив і що від нього треба → Глибокий аудит
 *   що робить команда     → Проєкт
 *
 * Було інакше: аудит був розрізаний на три вкладки («дані», «робота»,
 * «пакет»), файли клієнта й наші дельіверабли лежали в одних «Документах»
 * разом із підключенням аналітики, а сайт клієнта — теж у «Документах».
 * Сутності були перемішані, і жодна вкладка не відповідала на питання цілком.
 */
export const U_TABS: { id: UTab; l: string; hint?: string }[] = [
  { id: 'over', l: 'Огляд', hint: 'стан, наступний крок, заявки, історія' },
  { id: 'comp', l: 'Компанія', hint: 'профіль бізнесу, сайт, команда клієнта' },
  { id: 'express', l: 'Експрес-аудит', hint: 'що клієнт порахував сам' },
  { id: 'deep', l: 'Глибокий аудит', hint: 'питання · доступи · файли · результат' },
  { id: 'proj', l: 'Проєкти', hint: 'фактична робота з клієнтом' },
  { id: 'files', l: 'Файли', hint: 'клієнт → агентство і агентство → клієнт' },
  { id: 'kb', l: 'База знань клієнта', hint: 'матеріали й дані про ЦЬОГО клієнта, з джерелами; наші методики — в «Аудит і проєкти → Бібліотека»' },
  // Внутрішній шар окремо: нотатки команди — не матеріали про клієнта, і
  // лежати з ними поруч вони не мають. Клієнту їх не показують ніколи.
  { id: 'inner', l: 'Внутрішнє', hint: 'нотатки команди й оцінка модулів — не для клієнта' },
  { id: 'team', l: 'Наша команда' },
];

/**
 * Старі адреси вкладок лишились у закладках, листах і посиланнях у чаті.
 * Мовчки відкривати «Огляд» замість збереженої вкладки — це та сама втрата
 * контексту, тільки без повідомлення про помилку.
 */
const U_TAB_LEGACY: Record<string, UTab> = { data: 'deep', work: 'deep', pack: 'deep', docs: 'files' };
export const normalizeUTab = (raw: string | null | undefined): UTab => {
  const v = String(raw || '');
  if (U_TABS.some((t) => t.id === v)) return v as UTab;
  return U_TAB_LEGACY[v] || 'over';
};

/** Розділи всередині одного глибокого аудиту. Рівнів (T1–T4) тут немає. */
export type DeepSec = 'q' | 'access' | 'files' | 'result';
export const DEEP_SECS: { id: DeepSec; l: string; hint: string }[] = [
  { id: 'q', l: 'Питання', hint: 'анкета клієнта та уточнення' },
  { id: 'access', l: 'Доступи', hint: 'що клієнт відкрив і в якому стані' },
  { id: 'files', l: 'Файли', hint: 'вивантаження й звітність від клієнта' },
  { id: 'result', l: 'Результат', hint: 'прогін, оцінка, документ і пакет' },
];
