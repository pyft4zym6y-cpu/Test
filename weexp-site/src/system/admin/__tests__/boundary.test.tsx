...
/* Перехід «заявка → проєкт» перевіряється поведінкою — див. leadToProject.test.tsx:
   там картка справді рендериться в кожному зі станів. Тут лишався скан вихідника
   на рядок `cur !== 'done'`, тобто тест сторожив саме те правило, яке ми й
   прибрали: проєкт більше не чекає стадії «Завершена». */
