...
/**
 * Фіксована дата. Раніше тут стояв Date.now(): `staleDays` брав поточний час
 * напряму й ігнорував `now`, переданий у buildDigest, тож фікстури з конкретною
 * датою порівнювались із днем прогону і давали випадковий вік. Тепер `now`
 * проходить наскрізь — і зріз перевіряється на календарі, а не на «сьогодні».
 */
