...
export function accessBlock(): Block {
  ...
  return {
    key: 'access', cat: '14', title: 'Доступи до систем', role: 'Технічний',
...
const FILE_REQUESTS: { k: string; label: string; hint: string; tpl?: string; required?: boolean }[] = [
  ...
  /* Додано під конкретні документи, які без цих файлів рахуються зі слів:
     A3 (асортимент і ціни), A7 (реклама), Том B (контент), Том E (шлях). */
  { k: 'skusales', label: 'Продажі по SKU по місяцях (12–24 міс)', hint: 'ABC/XYZ, обіговість, dead stock — інакше асортимент оцінюється зі слів', required: true },
  { k: 'pricematrix', label: 'Матриця цін по каналах: сайт / МП / опт / роздріб', hint: 'конфлікт власного каналу з майданчиком — найчастіша прихована втрата маржі' },
  { k: 'promo', label: 'Промо-календар за 12 місяців', hint: 'глибина й частота знижок, синхронізація з рекламою' },
  { k: 'affiliate', label: 'Звіти по інфлюенсерах і affiliate', hint: 'канал без вимірювання або переоцінений, або закритий даремно' },
  { k: 'emails', label: 'Листи й повідомлення, які отримує клієнт', hint: 'третина шляху клієнта живе в пошті й з сайту не видна' },
  { k: 'editorial', label: 'Редполітика і tone of voice', hint: 'щоб правки текстів закріпились у процесі, а не лишились разовими' },
  { k: 'voc', label: 'Чати і розшифровки розмов з клієнтами (знеособлено)', hint: 'голос клієнта його словами — точніший за анкету про клієнта' },
];

/** Блок файлів — один список, видимий клієнту у вкладці «Файли». */
export function filesBlock(): Block {
  return {
    key: 'files', cat: '15', title: 'Файли та вивантаження', role: 'Фінанси',
...
/* ── Повний C-level фреймворк аудиту (13 модулів + доступи + файли) ──
...
export const AUDIT_FRAMEWORK: AuditTemplate = {
  ...
  blocks: [
    ...
    mod('12', 'organization', 'Organization: команда, процеси, AI', 'Власник', [
      ...
      ['block_rate', 'Оцініть ВАЖЛИВІСТЬ кожного напряму для вашого бізнесу (1–10)', 'rate10', { required: true, options: ['Економіка і фінанси', 'Ринок і бренд', 'Асортимент і ціни', 'Клієнт і утримання', 'Сайт і конверсія', 'SEO і органіка', 'Платна реклама', 'CRM і retention', 'Аналітика', 'Операції і логістика', 'Технології', 'Команда і процеси', 'Експансія і нові ринки'] }],
      ['org_file', 'Оргструктура (файл)', 'file'],
    ]),
    /* Модуль 13 — Expansion. Питання свідомо про те, чого ЩЕ НЕМАЄ: решта
       модулів описує роботу сьогодні, цей — вартість і ризик наступного кроку.
       Умовні питання висять тільки на `single`-відповіді (`focus`): AuditForm
       порівнює значення рядком, тож умова на `multi` не спрацювала б. */
    mod('13', 'expansion', 'Expansion: нові ринки й канали', 'Власник', [
      ['focus', 'Головний напрям експансії на найближчі 12 міс', 'single', { required: true, options: ['Нові країни', 'Нові маркетплейси', 'B2B / опт', 'Нові категорії', 'Офлайн / роздріб', 'Поки не вирішили'] }],
      ['plans', 'Які напрями розглядаєте взагалі (не лише головний)', 'multi', { options: ['Нові країни', 'Нові маркетплейси', 'B2B / опт', 'Нові категорії', 'Офлайн / роздріб', 'Власна торгова марка', 'Франшиза', 'Поки нікуди'] }],
      ['markets_new', 'Які саме країни й чому саме вони. Що вже перевіряли по цих ринках?', 'longtext', { condQKey: 'expansion_focus', condValue: 'Нові країни' }],
      ['mp_new', 'Які площадки розглядаєте. Юніт-економіка з комісією, логістикою й поверненнями порахована?', 'longtext', { condQKey: 'expansion_focus', condValue: 'Нові маркетплейси' }],
      ['b2b_new', 'Хто покупець в B2B, яка прайс-політика і чи є конфлікт з власною роздрібною ціною?', 'longtext', { condQKey: 'expansion_focus', condValue: 'B2B / опт' }],
      ['tried', 'Що з експансії вже пробували раніше і чому не пішло', 'longtext', { hint: 'невдала спроба — найдешевше джерело фактів про бар’єри' }],
      ['demand', 'Попит на новому ринку перевіряли даними?', 'single', { required: true, options: ['Так, є дані', 'Оцінювали на око', 'Ні'] }],
      ['demand_how', 'Чим саме перевіряли попит: джерела, період, обсяг', 'text', { condQKey: 'expansion_demand', condValue: 'Так, є дані' }],
      ['cost_entry', 'Оцінка вартості входу: бюджет, строки, хто веде напрям', 'text'],
      ['ops_ready', 'Що вже готове під новий ринок', 'multi', { options: ['Логістика', 'Оплати', 'Мова сайту', 'Підтримка', 'Повернення', 'Локальні ціни', 'Нічого'] }],
      ['legal', 'Юридичний контур нового ринку', 'multi', { hint: 'ПДВ і відповідальна особа блокують продаж раніше, ніж реклама', options: ['ПДВ-реєстрація / OSS-IOSS', 'GPSR (відповідальна особа в ЄС)', 'EPR (упаковка / електроніка)', 'Сертифікація товару', 'Локальна юрособа', 'Не вивчали'] }],
      ['constraint', 'Що зламається ПЕРШИМ при виході на новий ринок', 'single', { required: true, options: ['Склад і логістика', 'Гроші / касовий розрив', 'Команда', 'Платформа', 'Підтримка мовою ринку', 'Не знаємо'] }],
      ['owner', 'Хто персонально відповідає за напрям експансії', 'text'],
      ['sales_file', 'Вивантаження продажів у розрізі країн і площадок (файл)', 'file'],
    ]),
  ],
};
...
export const FRAMEWORK_PRESETS: { id: string; label: string; modules: string[] }[] = [
  { id: 'full', label: 'Єдиний повний фреймворк (13 модулів)', modules: ['business', 'market', 'product', 'customer', 'website', 'seo', 'acquisition', 'crm', 'analytics', 'operations', 'technology', 'organization', 'expansion'] },
];
