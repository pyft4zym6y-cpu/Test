...
export type FunnelState = {
  ...
  /** Запит на зустріч із результату експрес-аудиту: коли надіслано і що просили. */
  meetAt?: string;
  meetWhen?: string;      // зручний час словами (ранок / день / вечір, будні / вихідні)
  meetChannel?: string;   // канал: відеодзвінок / телефон / офіс
...
/** Обліковий запис клієнта в сервісі. */
export type ClientAccount = {
  id: string; email: string; confirmed: boolean; banned: boolean;
  lastSignIn: string | null; createdAt: string | null;
};
 ...
 *
 * Пошти в цьому шляху немає: ні запрошень, ні листів зі зміною пароля. Логін і
 * пароль задає менеджер і передає клієнту сам — доступ не залежить від того,
 * чи налаштований SMTP і чи дійшов лист.
 */
export async function clientAccessApi(
  action: 'create' | 'set_password' | 'revoke' | 'restore' | 'status' | 'list',
  payload: { email?: string; password?: string } = {},
): Promise<{ ok?: boolean; error?: string; users?: ClientAccount[] } & Partial<ClientAccess>> {
  try {
    const { data } = await supabase.auth.getSession();
    const token = data.session?.access_token;
