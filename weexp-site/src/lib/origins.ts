...
/**
 * Шляхи, які належать веденню проєкту.
 *
 * `/admin` тут НЕМАЄ свідомо. Спершу я переніс і його: розділяв «сайт» і «не
 * сайт». Але адмінка сайту — це заявки, які породжує сама сторінка, і вона
 * належить сайту так само, як форма, що їх створює. На app.* живе інше:
 * кабінет клієнта і консоль ведення проєкту (/manage).
 */
export const APP_PATHS = ['/cabinet', '/manage'] as const;

/**
 * Розділи, які існують ЛИШЕ українською.
 *
 * Кабінет двомовний — /en/cabinet це справжній маршрут, і англомовний клієнт
 * має лишитись англійською. Консоль ведення проєкту одномовна за конвенцією:
 * маршрута /en/manage не існує, і мовний префікс на ній веде просто у 404.
 */
const UK_ONLY = ['/manage'];
