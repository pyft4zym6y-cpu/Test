...
describe('блок на сторінках і сторінка статті', () => {
  ...
  it('блок стоїть в оболонці — і лише на сторінках блогу', () => {
    /*
     * В оболонці — бо доданий руками на кожну сторінку, він на трьох буде
     * забутий. Але БЕЗ УМОВИ він стояв на всіх двадцяти шести: 167 слів,
     * 1.2 екрана і пʼять посилань геть із воронки останнім блоком перед
     * підвалом — тобто одразу після фінального заклику до дії. Замір: на
     * /proof, /people, /services останній екран сторінки віддавався читанню.
     *
     * Тепер блок живе там, де читання і є дією. Сторож тримає обидві частини:
     * одне місце рендера — і умова, що це блог.
     */
    const shell = read('src/system/SystemShell.tsx');
    expect(shell, 'блок більше не рендериться з оболонки').toMatch(/<BlogTeaser\s*\/>/);
    expect(shell, 'блок знову показується поза блогом').toMatch(/isBlog\s*&&\s*<BlogTeaser/);
    expect(shell, 'умова «це блог» не звірена з адресою').toMatch(/isBlog\s*=\s*[^;]*'\/blog'/);
  });
  ...
  it('сторінка статті ставить власний заголовок у застосунку', () => {
    /*
     * Маршрут /blog/:slug не збігався з жодним ключем таблиці мета, і RouteSeo
     * не робив НІЧОГО: у статиці заголовок був правильний, а після переходу
     * всередині застосунку в укладці лишався заголовок сторінки, з якої
     * прийшли. Усі 44 статті ділили один заголовок — і це не було видно, бо
     * перевірялась лише статика.
     */
    const seo = read('src/lib/seo.tsx');
    expect(seo, 'RouteSeo не знає про статті блогу').toMatch(/blogMeta/);
    expect(seo, 'мета статті береться не з індексу блогу').toMatch(/bySlug/);
    // seoTitle має бути в легкому індексі, інакше застосунок і статика
    // назвуть ту саму сторінку по-різному.
    expect(read('scripts/blogIndex.mjs')).toMatch(/'seoTitle'/);
  });
