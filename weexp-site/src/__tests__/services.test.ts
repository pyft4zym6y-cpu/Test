...
describe('процес — один на всі формати', () => {
  ...
  it('кроки показуються з одного джерела', () => {
    /*
     * Сторінка цін, яка тримала власний перелік кроків у розмітці, з сайту
     * пішла: вона описувала ті самі три формати, що й сторінка послуг.
     * Процес лишився там, де його читають, — на головній і на кожній
     * сторінці формату.
     */
    for (const f of ['system/HomeBlocks.tsx', 'system/ServiceFormat.tsx'])
      expect(read(f), `${f}: кроки знову набрані в розмітці`).toContain('PROCESS.map');
  });
