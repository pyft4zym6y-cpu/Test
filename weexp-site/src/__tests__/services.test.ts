...
describe('процес — один на всі формати', () => {
  ...
  it('сторінка цін більше не тримає власного переліку кроків', () => {
    // Він жив просто в розмітці й був видимий лише тому, хто догортав сторінку
    // цін до середини. Тепер це джерело, і його показує ще й головна.
    const pricing = read('system/Pricing.tsx');
    expect(pricing, 'кроки знову набрані в розмітці').toContain('PROCESS.map');
    expect(pricing, 'у Pricing лишився власний масив форматів').not.toMatch(/const MODELS: Model\[\] = \[\s*\{/);
  });
});
