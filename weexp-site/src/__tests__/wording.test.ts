...
describe('одна дія — одна назва', () => {
  ...
  it('у кнопки на одну адресу — одна назва дії', () => {
     ...
     *
     * ВИНЯТКИ — /contact і /services/audit, і вони свідомі, не забуті. Власник
     * задає підписи під контекст: заявка називається «Знайти точки росту» в
     * герої, «Знайти можливості для росту» після переліку напрямів,
     * «Обговорити мій e-commerce» після опису першого кроку і «Залишити
     * заявку» в шапці; аудит — «Замовити комплексний аудит» після симптомів і
     * «Детальніше про формат 01» після експрес-розрахунку. Це його рішення про
     * тон, і сторож не місце, щоб його переголосувати.
     *
    ...
    const BY_CONTEXT = ['/contact', '/services/audit'];
    for (const l of links.filter((x) => x.button && !BY_CONTEXT.includes(x.to))) {
    ...
    // Винятки не мають розповзтись: адрес рівно стільки, скільки названо.
    const all = new Map<string, Set<string>>();
    for (const l of links.filter((x) => x.button)) {
      if (!all.has(l.to)) all.set(l.to, new Set());
      all.get(l.to)!.add(l.label);
    }
    expect([...all].filter(([, set]) => set.size > 1).map(([to]) => to).sort()).toEqual([...BY_CONTEXT].sort());
  });
