...
describe('низ сцени: рядок логотипів', () => {
  ...
  it('міряється МІСЦЕ ЗНИЗУ СЦЕНИ, а не висота самого рядка', () => {
    /*
     * Тут стояла вимога міряти getBoundingClientRect().height — і саме вона
     * була неправильною. На десктопі рядок стоїть bottom: 0, тож його висота
     * дорівнює місцю, яке він займає, і різниці не видно. На телефоні він
     * піднятий над нижньою панеллю (bottom: 58px): займає 104px, а height
     * повертав 46. Резерв був удвічі менший за потрібний, і «Безкоштовно ·
     * ~2 хв» знову лягало на логотипи — цього разу мовчки, бо механізм ніби
     * працював, а сторож ніби стеріг.
     *
     * Правильне питання: скільки місця рядок з'їдає знизу сцени. Тому —
     * різниця між низом сцени і верхом рядка.
     */
    expect(marquee).toMatch(/setProperty\('--sysx-marq-h'/);
    expect(marquee, 'знову міряємо висоту рядка замість зайнятого місця')
      .not.toMatch(/setProperty\('--sysx-marq-h',[^;]*getBoundingClientRect\(\)\.height/);
    expect(marquee).toMatch(/stage\.getBoundingClientRect\(\)\.bottom\s*-\s*el\.getBoundingClientRect\(\)\.top/);
    expect(marquee).toMatch(/ResizeObserver/);
  });

  it('стежить і за рядком, і за сценою', () => {
    // Висота сцени змінюється при повороті екрана й коли браузер ховає
    // адресний рядок: якщо спостерігати лише за рядком, резерв застрягне.
    expect(marquee).toMatch(/ro\.observe\(el\);\s*ro\.observe\(stage\)/);
  });
