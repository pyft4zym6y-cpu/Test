...
describe.skipIf(!built)('то, что видно в выдаче', () => {
  ...
  it('цена в описании доходит до выдачи целой', () => {
    /*
     * prerender подставлял значения строкой-заменой: `$1${desc}$2`. Как только
     * в описании появилась цена «$2,900», её `$2` стало ссылкой на вторую
     * группу — закрывающую кавычку. Атрибут обрывался на «Магазин », а хвост
     * описания уезжал в разметку. Ошибка была латентной годами: до страниц
     * форматов ни в одном title или description не было знака доллара.
     */
    const d = desc(html('services/audit/index.html'));
    expect(d, 'описание страницы формата снова обрезано на цене').toContain('$2,900');
    expect(d, 'вторая цена тоже должна дойти').toContain('$4,900');
  });
