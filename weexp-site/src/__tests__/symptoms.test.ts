...
describe('блок стоїть там, де вирішує', () => {
  it('на головній немає розповіді про нас — тільки розмова про клієнта', () => {
    /*
     * Правило було вужчим: сценарії мають стояти ПЕРЕД блоком процесу, бо
     * спершу людина впізнає свою проблему, а вже потім читає, як ми
     * працюємо. Порядок був правильний, але сам перелік блоків про нас на
     * головній — ні: перелік девʼяти експертиз, шість кроків процесу, «життя
     * після передачі», склад команди з девʼятнадцяти ролей. Замір: 4.7 екрана
     * з 12.2 і близько 500 слів із 1204 — між доказом і дією.
     *
     * Тому правило тепер не про порядок, а про склад: на головній лишається
     * те, що стосується клієнта (його числа, його симптом, його заперечення
     * і його дія), а розповідь про нас живе там, де на неї є питання.
     */
    const home = read('system/SystemInMotion.tsx');
    expect(home.indexOf('<Symptoms '), 'блоку сценаріїв немає на головній').toBeGreaterThan(0);
    for (const block of ['HowWeWork', 'AfterHandover', 'TeamStrip', 'HomeExpertise', 'SystemExplorer'])
      expect(home, `${block} повернувся на головну`).not.toContain(`<${block} `.trimEnd() + ' ');
    for (const block of ['HowWeWork', 'AfterHandover', 'TeamStrip', 'HomeExpertise', 'SystemExplorer'])
      expect(home, `${block} повернувся на головну`).not.toContain(`<${block} />`);
  });
