...
const ROUTES = [
   ...
   */
  { path: '/', og: 'home', title: 'WEEXP — комплексний e-commerce: більше прибутку з того самого трафіку',
    ...
    content: `<h1>Ваш магазин може заробляти більше. На тому самому трафіку.</h1><p>Збільшуємо прибуток інтернет-магазинів без збільшення рекламного бюджету — через CRO, UX/UI, оптимізацію клієнтського шляху, AI-технології, контент та системну роботу з даними.</p><h2>Три формати роботи</h2>${formatLinks('uk')}<p>${esc(SERVICES)}</p><h2>Що охоплює аудит</h2>${ul(SYSTEMS)}<p><a href="/services/audit">Що саме перевіряє аудит</a></p>` },
...
const FORMAT_PAGES = [
  {
    slug: 'audit',
    short: ['Аудит', 'Audit'],
    price: ['$2,900 / $4,900 · 4–6 тижнів', '$2,900 / $4,900 · 4–6 weeks'],
    promise: ['Карта: де саме витікають гроші, скільки це коштує на рік і що робити першим. Обираєте глибину: сам магазин ($2,900) чи весь відділ e-commerce ($4,900).',
              'A map: exactly where the money leaks, what it costs per year and what to fix first. You choose the depth: the store itself ($2,900) or the whole e-commerce department ($4,900).'],
    who: ['Кому підходить: у вас сильна внутрішня команда, потрібні не руки, а карта. Впровадження та результат — ваша команда. 100% вартості аудиту зараховується в перший місяць управління під ключ, якщо старт упродовж 30 днів.',
          'Who it fits: you have a strong in-house team and need a map, not hands. Implementation and the result — your team. 100% of the audit fee is credited to the first month of managed delivery if you start within 30 days.'],
  },
  {
    slug: 'consulting',
    short: ['Консалтинг', 'Consulting'],
    price: ['$50/год · мінімум 30 год/міс', '$50/hr · minimum 30 hrs/mo'],
    promise: ['Зовнішній архітектор для вашої команди: що робити, в якому порядку і чи зроблено якісно. Рахунок не буває менше $1,500 на місяць.',
              'An external architect for your team: what to do, in what order and whether it is done well. The invoice is never below $1,500 per month.'],
    who: ['Кому підходить: у вас є виконавці та проджект-менеджер. Якість рішень і контроль — ми; виконання руками та результат — ваша команда. Старт — після аудиту, початковий термін 3 місяці.',
          'Who it fits: you have doers and a project manager. Decision quality and control — us; hands-on execution and the result — your team. Starts after the audit, initial term 3 months.'],
  },
  {
    slug: 'managed',
    short: ['Управління під ключ', 'Managed delivery'],
    price: ['від $4,900/міс · 6–12 місяців', 'from $4,900/mo · 6–12 months'],
    promise: ['Проєкт ведемо ми — план, люди, бюджет і фінальна відповідальність за результат.',
              'We run the project — plan, people, budget and final accountability for the result.'],
    who: ['Кому підходить: нема кому вести це зсередини, потрібен результат, а не поради. Пілот — перші 3 місяці з фіксованими KPI першої хвилі, далі 6–12 місяців. Старт — тільки після аудиту.',
          'Who it fits: there is nobody to run this in-house and you need a result, not advice. A 3-month pilot with fixed first-wave KPIs, then 6–12 months. Starts only after the audit.'],
  },
];

/*
 * Тіло сторінки формату — ОДНА функція на обидві мови.
 *
 * Доти англійських сторінок форматів не існувало взагалі: цикл EN нижче брав
 * мету лише з seo-data.json, а форматів там немає, — тож три адреси мовчки
 * випадали і зі статики, і з sitemap. Поки їх ніщо не показувало, це не
 * боліло. Мега-меню тепер веде на них із КОЖНОЇ англійської сторінки: посилання
 * є, сторінки для краулера немає.
 *
 * Коротка назва — та сама, що в H1 на сторінці («Аудит», а не «Аудит
 * онлайн-продажів»). Довга лишається в <title> і описі, де вона й працює на
 * пошук. Доти робот бачив одну назву, людина — іншу.
 */
const formatBody = (f, lang) => {
  const i = lang === 'en' ? 1 : 0;
  const head = `<h1>${esc(f.short[i])}</h1><p>${esc(f.promise[i])}</p><p><b>${esc(f.price[i])}</b></p><p>${esc(f.who[i])}</p>`;
  const audit = f.slug !== 'audit' ? ''
    : i ? `<h2>What the deep audit covers</h2><p>This is not a website check: we take apart the whole e-commerce structure — from the commercial model and analytics to operations and technology.</p>${ul(SYSTEMS_EN)}`
  ...
  const tail = i ? `<p><a href="/en/services">All three ways to work</a></p>`
                 : `<p><a href="/services">Усі три формати роботи</a> · <a href="/pricing">умови поруч</a></p>`;
  return head + audit + tail;
};

/*
 * Заголовок і опис — зі спільної таблиці seo-data.json, яку читає і застосунок.
 *
 * Доти вони були написані тут окремо, і в статиці стояло «Аудит онлайн-продажів
 * · WEEXP», а застосунок одразу після завантаження переписував укладку на
 * «Аудит — формат співпраці з WEEXP». Googlebot виконує JS, тобто ретельно
 * дібраний пошуковий заголовок не доживав до видачі жодного разу.
 *
 * Опис написаний окремо від тексту сторінки, а не склеєний із нього. Перша
 * версія збирала його як `${promise} ${price}.` — виходило 188 символів: у
 * видачі обрізалось посеред слова, та ще й з малої літери після крапки.
 */
for (const f of FORMAT_PAGES) {
  const m = SEO.routes[`/services/${f.slug}`];
  ROUTES.push({
    path: `/services/${f.slug}`, og: 'pricing',
    title: m.uk[0], desc: m.uk[1], content: formatBody(f, 'uk'),
  });
}

/** Формати за адресою — щоб цикл EN нижче знайшов їх так само, як експертизи. */
const FORMAT_BY_PATH = Object.fromEntries(FORMAT_PAGES.map((f) => [`/services/${f.slug}`, f]));
...
for (const r of ROUTES) {
  ...
  if (!m) continue;
  /*
   * Формати мали мету лише в цьому скрипті, а не в спільній таблиці, — і саме
   * тому три англійські адреси не існували ні в статиці, ні в sitemap, хоча в
   * застосунку малювались. Поки на них ніщо не вело, це не боліло; мега-меню
   * тепер веде на них із КОЖНОЇ англійської сторінки. Тіло беремо з того
   * самого FORMAT_PAGES, що й українське: одне джерело на дві мови.
   */
  const f = FORMAT_BY_PATH[key];
  if (f) {
    EN.push({ path: `/en${key}`, og: r.og, lang: 'en', title: m.en[0], desc: m.en[1], content: formatBody(f, 'en') });
    continue;
  }
