...
const R = (id: string, kind: ReqKind, tier: Tier, title: string, why: string, assetId?: string): Requirement =>
  ({ id, kind, title, why, tier, assetId });
...
export const BLOCKS: AuditBlock[] = [
  ...
  },
  {
    ...
    minTier: 1,
    reqs: [
      R('pg-crawl', 'access', 1, 'Публичный сайт (обход ключевых типов)', 'Разбор главной, категории, карточки, корзины.'),
  ...
  },
  {
    ...
    reqs: [
      ...
      R('mch-sales', 'document', 3, 'Продажи по SKU за 12 мес', 'Что реально продаётся vs что на витрине.', 'tpl-orders'),
    ],
  },
  {
    id: 'cro',
  ...
  },
  {
    ...
    minTier: 2,
    reqs: [
      R('an-ga4', 'access', 2, 'Google Analytics 4 (просмотр)', 'Аудит настройки, событий, воронок.', 'ins-ga4'),
  ...
  },
  {
    ...
    minTier: 3,
    reqs: [
      R('ue-orders', 'document', 3, 'Выгрузка заказов 24 мес', 'AOV, частота, возвраты, повторные.', 'tpl-orders'),
