...
/**
 * Вопросы про причины / источники / критерии / барьеры: несколько ответов
 * действительны одновременно — принудительно мультивыбор.
 */
const MULTI_OVERRIDE = new Set([
  'CO-001', // почему проект именно сейчас — триггеров бывает несколько
  'PX-003', // чем подтверждены причины покупки
  'PX-019', // как принимается решение о новом продукте
  'CU-002', // на каких данных построены персоны
  'MP-002', // почему не выходили на маркетплейсы
  'IN-002', // почему не хотите в ЕС
  'IN-044', // основание выбора товаров для ЕС
  'AI-126', // что мешает внедрять AI
  'PL-002', // кто занимается разработкой
  'PL-006', // где живут товарные данные
  'BI-003', // как команда узнаёт о бренд-платформе
]);

export const QUESTIONS = (questionsRaw as Question[]).map((q) =>
  MULTI_OVERRIDE.has(q.id) ? { ...q, type: 'Мультивыбор' } : q,
);
...
export const ACCESS_GUIDES: Record<string, string> = {
  ...
  'EX-01': 'Регламенты, описания процессов, схема систем — любыми файлами в защищённую папку.',
  'EX-02': 'Мы пришлём готовую анкету (5 минут на прохождение) — от вас рассылка по базе клиентов.',
  'EX-03': 'Figma: Share → пригласить e-mail с правами «view». Или экспорт макетов файлами.',
  'EX-04': 'Тарифы перевозчиков, время сборки заказа, стоимость упаковки — файлом или в комментарии.',
  'EX-05': 'Перечень юрлиц, налоговые режимы, статус VAT/OSS — файлом в защищённую папку.',
  'EX-06': 'Купленные отчёты по рынку/нише, если есть — PDF в папку.',
};
