...
function Logo() {
  return (
    <Link to="/" className="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
      ...
      <span className="font-pixel text-[0.6rem] leading-none pt-0.5 text-[#F3F5F7]">
        WEEXP<span className="acid">·OS</span>
      </span>
    </Link>
  );
}
...
export default function Nav() {
  ...
  const linkCls = (isActive: boolean) =>
    `text-xs 2xl:text-sm tracking-wide uppercase transition-colors hover:text-white ${
      isActive ? 'acid' : 'text-[#C2C6CC]'
    }`;
    ...
    <>
      <header
        className={`fixed top-0 inset-x-0 z-50 transition-colors duration-300 ${
          scrolled ? 'bg-[#0A0608]/90 backdrop-blur-md border-b border-white/10' : ''
        }`}
      >
        <nav className="max-w-7xl mx-auto flex items-center justify-between px-5 sm:px-6 md:px-10 py-5">
              ...
              l.to === '/cases' ? (
                ...
                <li key={l.to} className="relative group">
                  ...
                  <div className={`absolute left-1/2 -translate-x-1/2 top-full pt-3 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto group-focus-within:opacity-100 group-focus-within:pointer-events-auto transition-opacity duration-200${casesOpen ? ' opacity-100 pointer-events-auto' : ''}`}>
                    <div className="bg-[#0A0608]/97 backdrop-blur-md border border-white/12 min-w-[260px] py-2">
                      {CASE_LINKS.map((c) => (
                        <Link
                          key={c.to}
                          to={c.to}
                          ...
                          onMouseLeave={sayIdle}
                          className="flex items-baseline justify-between gap-4 px-5 py-2.5 text-xs uppercase tracking-wide text-white/70 hover:text-white hover:bg-white/5 transition-colors"
                        >
                          <span>{c.label}</span>
                          <span className="font-mono text-[0.62rem] acid">{c.num}</span>
                        </Link>
                      ))}
                      <Link
                        to="/cases"
                        className="block px-5 py-2.5 mt-1 border-t border-white/10 text-xs uppercase tracking-wide acid hover:bg-white/5 transition-colors"
                      >
                        Всі кейси →
                      </Link>
                    </div>
          ...
          <div className="flex items-center gap-3">
            <Link
              ...
              className="hidden xl:inline-block border border-white/25 px-5 py-2 text-xs tracking-wider uppercase text-[#E8EAED] hover:border-white/60 transition-colors"
            >
              Забронювати сесію
            </Link>
            <a
              ...
              className="hidden xl:inline-block bg-[var(--acid)] px-5 py-2 text-xs font-bold tracking-wider uppercase text-white hover:bg-[var(--acid-deep)] transition-colors"
            >
              Заповнити бриф
            </a>
            <Link
              ...
              className="xl:hidden bg-[var(--acid)] px-3.5 py-2 font-mono text-[0.62rem] font-bold uppercase tracking-wider text-white"
            >
              Аудит →
            </Link>
            <button
              ...
              className="xl:hidden p-2 border border-white/20 text-[#E8EAED] hover:border-white/50 transition-colors"
            >
              <Menu size={22} />
            </button>
          </div>
      ...
      <div
        ...
        className={`menu-overlay fixed inset-0 z-[60] bg-[#0A0608]/97 backdrop-blur-md flex flex-col overflow-y-auto ${
          menuOpen ? 'opacity-100 visible pointer-events-auto' : 'opacity-0 invisible pointer-events-none'
        }`}
      >
        <div className="flex items-center justify-between px-6 h-[68px] border-b border-white/[0.08] shrink-0">
          <Logo />
          <button
            type="button"
            aria-label="Закрити меню"
            onClick={() => setMenuOpen(false)}
            className="p-2 -mr-2 border border-white/20 text-[#E8EAED] hover:border-white/50 transition-colors"
          >
            <X size={20} />
          </button>
        </div>
        <nav className="flex-1 flex flex-col px-6 pt-5 pb-8 overflow-y-auto">
            ...
            {LINKS.map((l, i) => (
              <div key={l.to}>
                <NavLink
                  ...
                  onClick={() => setMenuOpen(false)}
                  className={({ isActive }) =>
                    `menu-link flex items-baseline gap-4 py-[0.68rem] border-b border-white/[0.07] ${
                      isActive ? 'acid' : 'text-[#F3F5F7]'
                    } ${menuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`
                  }
                  style={{ transitionDelay: menuOpen ? `${70 + i * 45}ms` : '0ms' }}
                >
                  <span className="font-pixel text-[0.5rem] acid w-7 shrink-0 opacity-80">
                    0{i + 1}
                  </span>
                  <span className="font-cinema font-extrabold text-[1.5rem] leading-tight tracking-tight">
                    {l.label}
                  </span>
                </NavLink>
                {l.to === '/cases' && (
                  <div
                    className={`menu-link flex flex-col border-b border-white/[0.07] py-2 ${
                      menuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                    }`}
                  ...
                  >
                    {CASE_LINKS.map((c) => (
                      <Link
                        ...
                        to={c.to}
                        onClick={() => setMenuOpen(false)}
                        className="flex items-baseline justify-between gap-3 pl-11 pr-1 py-[0.42rem] text-[0.78rem] uppercase tracking-wide text-white/55 hover:acid transition-colors"
                      >
                        <span className="truncate">{c.label}</span>
                        <span className="font-mono text-[0.66rem] acid shrink-0">{c.num}</span>
                      </Link>
                    ))}
                  </div>
                )}
          ...
          >
            <Link
              to="/contact"
              onClick={() => setMenuOpen(false)}
              className="block bg-[var(--acid)] text-white text-center font-mono text-sm font-bold uppercase tracking-[0.12em] px-8 py-4"
            >
              Забронювати сесію →
            </Link>
            <a
              ...
              className="block border border-white/30 text-[#F3F5F7] text-center font-mono text-sm font-bold uppercase tracking-[0.12em] px-8 py-4"
            >
              Заповнити бриф →
            </a>
            <div className="flex items-center justify-between font-mono text-[0.68rem] text-[#8A9099]">
              <a href="tel:+380999188260" className="hover:text-white transition-colors">
                +38 099 918 82 60
              </a>
              <a
