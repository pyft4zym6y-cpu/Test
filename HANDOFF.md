...
- ~~Пререндер/SSG сайта~~ — СДЕЛАНО: `scripts/prerender.mjs` (после `npm run build`
  снимает все маршруты Chromium'ом в `dist/<route>/index.html`; Vercel отдаёт
  статику прежде rewrites). Запускать локально/в CI с браузером, НЕ в Vercel build.
  Прогонять перед каждым деплоем, если менялся контент.
- ~~Уникальные title для каждого кейса~~ — СДЕЛАНО (CaseDetailPage подписывает
  title/description как BlogPostPage). Остались og-картинки под страницы.
