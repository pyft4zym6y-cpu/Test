...
| | Хто | Як заходить |
|---|---|---|
| Клієнт | акаунт заводить менеджер вручну | email + пароль, або Google |
| Команда | ролі з `app_metadata` (див. `/api/team`) | так само |

Самореєстрації немає, і **пошти сервіс не надсилає взагалі** — ні запрошень, ні
листів зі зміною пароля. Логін (email клієнта) і пароль задає менеджер і передає
клієнту сам. Це знімає залежність від SMTP: доступ працює одразу й не ламається,
коли лист не дійшов чи потрапив у спам. Ціна — пароль треба передати безпечним
каналом власноруч.

Де це робиться:
* **Консоль → Облікові записи** — усі доступи одним списком: хто має вхід, хто
  заходив, кому змінити пароль чи забрати доступ.
* **Картка клієнта → Огляд** — те саме для конкретного клієнта, не виходячи з
  картки, з якою й так працюєш.

Забрати доступ — бан, а не видалення: дані проєкту звʼязані з `user_id`, і
видалення акаунта лишило б без власника саме той запис, заради якого велась
робота.

Усе йде через `/api/client-access`. Ендпоінт **не видає ролей узагалі** і **не
має влади над акаунтами команди** — інакше менеджер міг би надіслати туди пошту
super-адміна й забрати найвищий доступ, маючи найнижчий. Ролі команди — окремо,
у `/api/team`, і лише super. Тести: `src/__tests__/clientAccess.test.ts`
(перевірені на падіння: гард прибрано → тест червоний).

### Перший адміністратор

Заводити клієнтів уміє консоль, але щоб у неї увійти, потрібен акаунт із
паролем — а зробити його з UI нема кому. Разова операція, разовий інструмент:

```bash
SUPABASE_URL=https://<projeсt>.supabase.co \
SUPABASE_SERVICE_ROLE_KEY=<service_role> \
node scripts/create-account.mjs --email you@weexp.agency --role super
```

Пароль скрипт спитає в терміналі й не прийме аргументом: аргументи видно в `ps`
і вони лишаються в історії оболонки. Без `--role` створюється клієнт (без ролі).
Бутстрап-ендпоінта свідомо немає: адреса, яка створює адміністратора, рано чи
пізно виявляється відкритою.

Те саме можна зробити руками: Supabase → Authentication → Users → Add user
(email + пароль + Auto Confirm). Ролі `super` для
`pashasidorenko18@gmail.com` і `hello@weexp.agency` видаються бутстрап-списком.

**Що треба у Vercel:** `SUPABASE_SERVICE_ROLE_KEY` (Supabase → Settings → API →
service_role) — **без** префікса `VITE_`. З `VITE_` ключ потрапляє в JS-бандл, а
це повний доступ до бази для будь-кого, хто відкрив devtools. SMTP більше не
потрібен.
