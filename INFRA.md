...
| | Хто | Як заходить |
|---|---|---|
| Клієнт | тільки за запрошенням менеджера | email + пароль, або Google |
| Команда | ролі з `app_metadata` (див. `/api/team`) | так само |

Самореєстрації в сервісі немає свідомо: там лежать дані проєктів, і відкритий
вхід означав би, що будь-хто з інтернету заводить у ньому акаунт. Менеджер
відкриває доступ із картки клієнта (Огляд → «Доступ до сервісу»): лист із
запрошенням, або пароль вручну, якщо пошта клієнта не приймає наших листів.

Усе це йде через `/api/client-access`. Ендпоінт **не видає ролей узагалі** і
**не має влади над акаунтами команди** — інакше менеджер міг би надіслати туди
пошту super-адміна й забрати найвищий доступ, маючи найнижчий. Ролі команди —
окремо, у `/api/team`, і лише super. Тести: `src/__tests__/clientAccess.test.ts`
...
це повний доступ до бази для будь-кого, хто відкрив devtools. Для листів
(запрошення, зміна пароля) у Supabase має бути налаштований SMTP, інакше
лишається шлях «пароль вручну».
